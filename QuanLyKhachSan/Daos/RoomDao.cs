...
namespace QuanLyKhachSan.Daos
{
    public class RoomDao
    {
        ...
        public List<Room> SearchByChildrenAndAdult(int page, int pagesize, int numberChildren, int numberAdult)
        {
            // var arrIdRoom = myDb.bookings.Where(x => x.status == 0 || x.status == 1).Select(x => x.idRoom).Distinct().ToList();
            var allId = myDb.rooms.Select(x => x.idRoom).ToList();
            //Except(arrIdRoom).
            var ids = allId.ToList();
            return myDb.rooms.Where(x => ids.Contains(x.idRoom) && x.numberAdult >= numberAdult && x.numberChildren >= numberChildren).ToList().Skip((page - 1) * pagesize).Take(pagesize).ToList();
        }
        public int GetNumberRoomByChildrenAndAdult( int numberChildren, int numberAdult)
        {
           // var arrIdRoom = myDb.bookings.Where(x => x.status == 0 || x.status == 1).Select(x => x.idRoom).Distinct().ToList();
            var allId = myDb.rooms.Select(x => x.idRoom).ToList();
            //.Except(arrIdRoom)
            var ids = allId.ToList();
            int total = myDb.rooms.Where(x => ids.Contains(x.idRoom) && x.numberAdult >= numberAdult && x.numberChildren >= numberChildren).ToList().Count;
            int count = 0;
            count = total / 3;
            if (total % 3 != 0)
            {
